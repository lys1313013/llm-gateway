...
const exposedSelectCols = `id, model_id, owned_by, is_active, last_openai_test_time, last_anthropic_test_time, create_time, update_time`
...
func UpdateExposedModel(ctx context.Context, id int, in UpdateExposedModelInput) (*models.ExposedModel, error) {
	row := mustHavePool().QueryRow(ctx, `
		UPDATE exposed_model
		   SET model_id = $2, owned_by = $3, is_active = $4, update_time = CURRENT_TIMESTAMP
		 WHERE id = $1
		RETURNING `+exposedSelectCols,
		id, in.ModelID, in.OwnedBy, in.IsActive)
	m, err := scanExposedModel(row)
	if err != nil {
		if errors.Is(err, ErrNotFound) {
			return nil, nil
